...
func (seat Seat)String() string {
  return fmt.Sprintf("%s %d [%d,%d]",seat.descriptor,seat.number(),seat.row,seat.col)
}
...
func main() {
  ...
  sort.Sort(BySeat(passes))

  fmt.Printf("First seat is %v; last is %v:\n",passes[0],passes[len(passes)-1])

  var last Seat
  for _, seat := range(passes) {
    if last == (Seat{}) {
      last = seat
      continue
    }
    fmt.Println("last",last,"current",seat)
